...
bool WatchDaemon::acquireDirectoryLock(const QString &directory, qint64 &conflictingPid)
{
    ...
    QString lockPath = dir.filePath(QStringLiteral(".bentopack-watch.lock"));
    ...
    QString pidPath = dir.filePath(QStringLiteral(".bentopack-watch.pid"));
...
void WatchDaemon::releaseDirectoryLock()
{
    ...
    if (!m_lockedDirectory.isEmpty()) {
        ...
        dir.remove(QStringLiteral(".bentopack-watch.pid"));
        dir.remove(QStringLiteral(".bentopack-watch.lock"));
...
bool WatchDaemon::isIgnoredPath(const QString &path) const
{
    ...
    if (fn.startsWith(QStringLiteral(".bentopack-watch")) ||
                                     ...
                                     bool daemonMode)
{
    ...
    if (!acquireDirectoryLock(primaryDir, conflictingPid)) {
        return CliResult::error(ExitLockConflict,
            QStringLiteral("Lock conflict: Another bentopack-cli watch daemon is already running for directory: %1 (PID: %2).")
