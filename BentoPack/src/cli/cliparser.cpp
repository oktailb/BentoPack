...
QString CliParser::helpText()
{
    return QStringLiteral(
        "BentoPack CLI - Headless Texture Packing & CI/CD Asset Pipeline\n\n"
        "Usage:\n"
        "  bentopack-cli [command] [options] <files...>\n"
        "  bentopack-cli --sheet <file> --data <file> [options] <files...>\n"
        ...
        "  bento                       Inspect or export native .bento projects (alias: ssp)\n\n"
...
CliResult CliParser::dispatchCommand(const QStringList &cleanArgs)
{
    ...
    if (!cleanArgs.isEmpty()) {
        ...
        if (subCmd == QStringLiteral("bento") || subCmd == QStringLiteral("ssp")) {
            return NativeCommands::executeBento(cleanArgs.mid(1));
        }
