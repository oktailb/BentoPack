...
QMap<QString, QString> LicenseManager::complianceMetadata()
{
    ...
    if (IntegrityGuard::isTampered()) {
        meta.insert(QStringLiteral("Generator"), QStringLiteral("SpriteStudio %1 (Tampered Build)").arg(toolStr));
        meta.insert(QStringLiteral("X-SS-Integrity"), QStringLiteral("Tampered-%1-Binary-Circumvention").arg(toolStr));
        meta.insert(QStringLiteral("X-SpriteStudio-Tool"), toolStr);
        meta.insert(QStringLiteral("X-SpriteStudio-Notice"),
                    QStringLiteral("UNAUTHORIZED CIRCUMVENTED BUILD - Copyright & DMCA Violation"));
    } else if (isCommercial()) {
        meta.insert(QStringLiteral("Generator"), QStringLiteral("SpriteStudio %1").arg(toolStr));
        meta.insert(QStringLiteral("X-SpriteStudio-Edition"), QStringLiteral("Commercial"));
        meta.insert(QStringLiteral("X-SpriteStudio-Tool"), toolStr);
    } else {
        meta.insert(QStringLiteral("Generator"), QStringLiteral("SpriteStudio %1 Community Edition").arg(toolStr));
        meta.insert(QStringLiteral("X-SpriteStudio-Tool"), toolStr);
        meta.insert(QStringLiteral("X-SpriteStudio-License"), QStringLiteral("Community-Exemption-Under-1M-%1").arg(toolStr));
        if (isCli) {
            meta.insert(QStringLiteral("X-SpriteStudio-Notice"),
...
void LicenseManager::applyWatermark(QImage &image)
{
    ...
    if (IntegrityGuard::isTampered()) {
        image.setText(QStringLiteral("Generator"), QStringLiteral("SpriteStudio %1 (Tampered Build)").arg(toolStr));
        image.setText(QStringLiteral("X-SS-Integrity"), QStringLiteral("Tampered-%1-Binary-Circumvention").arg(toolStr));
        image.setText(QStringLiteral("X-SpriteStudio-Tool"), toolStr);
        image.setText(QStringLiteral("X-SpriteStudio-Notice"),
                      QStringLiteral("UNAUTHORIZED CIRCUMVENTED BUILD - Copyright Violation"));
    } else if (isCommercial()) {
        image.setText(QStringLiteral("Generator"), QStringLiteral("SpriteStudio %1").arg(toolStr));
        image.setText(QStringLiteral("X-SpriteStudio-Tool"), toolStr);
    } else {
        image.setText(QStringLiteral("Generator"), QStringLiteral("SpriteStudio %1 Community Edition").arg(toolStr));
        image.setText(QStringLiteral("X-SpriteStudio-Tool"), toolStr);
        image.setText(QStringLiteral("X-SpriteStudio-License"), QStringLiteral("Community-Exemption-Under-1M-%1").arg(toolStr));
        if (isCli) {
            image.setText(QStringLiteral("X-SpriteStudio-Notice"),
                          QStringLiteral("Evaluation & indie usage (<1M$ revenue exemption). Commercial CLI Automation / CI pipeline license required for automated build pipelines or above threshold."));
        } else {
...
void LicenseManager::applyWatermark(QJsonObject &metaObj)
{
    ...
    if (IntegrityGuard::isTampered()) {
        metaObj[QStringLiteral("app")] = QStringLiteral("SpriteStudio %1 (Tampered Build)").arg(toolStr);
        ...
        metaObj[QStringLiteral("app")] = QStringLiteral("SpriteStudio %1").arg(toolStr);
        metaObj[QStringLiteral("tool")] = toolStr;
        ...
        metaObj[QStringLiteral("app")] = QStringLiteral("SpriteStudio %1 Community Edition").arg(toolStr);
        metaObj[QStringLiteral("tool")] = toolStr;
...
QString LicenseManager::watermarkHeaderComment()
{
    ...
    if (IntegrityGuard::isTampered()) {
        return QStringLiteral("; WARNING: Generated by Tampered / Circumvented SpriteStudio %1 binary (Unlicensed / Copyright Violation)\n"
                              "; X-SS-Integrity: Tampered-%1-Binary-Circumvention\n\n").arg(toolStr);
    } else if (isCommercial()) {
        return QStringLiteral("; Generated by SpriteStudio %1\n\n").arg(toolStr);
    } else {
        if (isCli) {
            return QStringLiteral("; Generated by SpriteStudio CLI Community Edition (Evaluation & Indie <1M$ Revenue Exemption)\n"
            ...
            return QStringLiteral("; Generated by SpriteStudio GUI Community Edition (Evaluation & Indie <1M$ Revenue Exemption)\n"
