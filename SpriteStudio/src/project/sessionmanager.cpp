...
QString SessionManager::sessionsRootPath()
{
    ...
    return QDir(tempBase).filePath(QStringLiteral("SpriteStudio/sessions"));
}
...
bool SessionManager::openSessionFromSsp(const QString &sspPath, QString *errorMsg)
{
    if (sspPath.isEmpty() || !QFile::exists(sspPath)) {
        if (errorMsg) *errorMsg = tr("Project file does not exist: %1").arg(sspPath);
        return false;
    }

    if (!startNewSession(QFileInfo(sspPath).absoluteFilePath())) {
        if (errorMsg) *errorMsg = tr("Failed to initialize session directory.");
        return false;
    }

    if (!unpackZip(sspPath, m_sessionDir, errorMsg)) {
...
#ifdef HAVE_LIBGIT2
    gitInit();
    // Only commit initial load if the repository has no previous commits (e.g. legacy .ssp)
    if (gitLog().isEmpty()) {
        gitCommit(QStringLiteral("Initial project load from .ssp"));
    }
#endif
...
static bool addDirectoryToZip(mz_zip_archive &zip, const QDir &dir, const QString &baseDir)
{
    ...
    for (const QFileInfo &entry : entries) {
        // Do not pack temporary session lock file into .ssp archive
...
bool SessionManager::saveSessionToSsp(const QString &targetSspPath, QString *errorMsg)
{
    ...
    if (targetSspPath.isEmpty()) {
        if (errorMsg) *errorMsg = tr("Target path cannot be empty.");
        return false;
    }

    QFileInfo targetInfo(targetSspPath);
    QDir().mkpath(targetInfo.absolutePath());

    QString tempSspPath = targetSspPath + QStringLiteral(".tmp");

    // Remove old temp file if it exists
    if (QFile::exists(tempSspPath)) {
        QFile::remove(tempSspPath);
    }
...
#ifdef HAVE_LIBGIT2
    gitCommit(QStringLiteral("Project saved to .ssp: ") + targetInfo.fileName());
#endif

    // Pack scratch session directory to temp file
    if (!packZip(m_sessionDir, tempSspPath, errorMsg)) {
        QFile::remove(tempSspPath);
        return false;
    }

    // Atomic replacement: remove target if exists and rename temp -> target
    if (QFile::exists(targetSspPath)) {
        if (!QFile::remove(targetSspPath)) {
            if (errorMsg) *errorMsg = tr("Failed to overwrite existing file: %1").arg(targetSspPath);
            QFile::remove(tempSspPath);
            return false;
        }
    }

    if (!QFile::rename(tempSspPath, targetSspPath)) {
        if (errorMsg) *errorMsg = tr("Failed to atomically rename %1 to %2").arg(tempSspPath, targetSspPath);
        return false;
    }

    emit sessionSaved(targetSspPath);
    return true;
}
...
bool SessionManager::gitCommit(const QString &message)
{
    ...
    if (git_signature_now(&sig, actAuthor.toUtf8().constData(), actEmail.toUtf8().constData()) != 0 || !sig) {
        git_signature_now(&sig, "SpriteStudio", "spritestudio@local");
    }
...
QString SessionManager::authorName() const
{
    ...
    return detectedName.isEmpty() ? QStringLiteral("SpriteStudio") : detectedName;
}
...
QString SessionManager::authorEmail() const
{
    ...
    return detectedEmail.isEmpty() ? QStringLiteral("spritestudio@local") : detectedEmail;
}
